export const projects = [
    {
      title: "Weather API",
      subtitle: "HTML/CSS, JavaScript and Express.js",
      description:
        " Used OpenWeather API to build an app that displays the weather in different cities around the world. Developed the interface using Front-end tools & used Node to get user input        ",
      image: "./weather.gif",
      link: "https://protected-fjord-75733.herokuapp.com/",
    },
    {
      title: "Clicker game",
      subtitle: "HTML/CSS, and JavaScirpt",
      description:
        "Developed a Front-end page to create a clicking game. Used JavaScript to handle clicks from a user.",
      image: "./clicker.gif",
      link: "https://javiermajano.github.io/ClickerGame/",
    },
    {
      title: "Concsiour Caloric Tracker",
      subtitle: " node.js, react, express, postgresql ",
      description:
        "Developed a Full Stack web application ",
      image: "./clicker.gif",
      link: "https://github.com/CTP-Calorie/project-starter/",
    },

  ];
  export const skills = [
    "JavaScript",
    "React",
    "Node.js",
    "Express.js",
    "HTML/CSS",
    "C++"
  ];